...
- Same-process discovery of Controller, GCM, and one AI Gateway fallback still reads Obsidian's private plugin registry because those peer plugins expose APIs on plugin instances. Watchlist also publishes `app.tpsWatchlist`, and the AI fallback consumes `app.tpsAiGateway`; both are App-object monkeypatch contracts. Notifier v2 no longer uses that route, but its canonical client retains one explicitly isolated v1 private-registry bridge until mixed-version devices are retired.
...
- `npm run test:core` runs 68 tests: 47 core/identity/integration regressions plus 21 executable notification tests covering null-prototype/reserved keys, legacy-empty loading, future/malformed/contradictory fail-closed preservation, startup recovery, deduped-marker no-send behavior, exact v1 state round-trip and attempt-ID CAS, deterministic bounded pruning, capacity refusal, durable orchestration ordering, preparation/terminal-save failures, pre/post-I/O ownership conflicts, unload/save-queue fencing, classified non-throwing outcomes, blocked-ledger suppression, v2 preference and certainty mappings, exact unavailable invalidation, mixed-version v1 behavior, bounded consumer-timeout uncertainty, no same-occurrence fallback, malformed receipts, legacy rejection, and throwing accessor guards.
- `npm test` runs focused core tests and the production TypeScript/esbuild build.
- Core state/lease/settlement/effect and notification client/ledger/orchestration behavior have direct executable coverage. Several full plugin/Obsidian orchestration checks remain source-structure assertions rather than a complete fake-Obsidian integration harness.
